...
public class MazeVisualizer extends JApplet 
{
    ...
    /**
     * Overloaded the addPath method so it accepts arraylists
     * @param nodeList A array list
     */
    public void addPath(ArrayList<Integer>nodeList) 
    {
        LinkedList<Integer> path = new LinkedList<>();
        for(Integer i : nodeList) 
        {
    ...
    public void paint(Graphics g) 
    {
        ...
        for(int i = 1; i <= N; i++ ) 
        {
            for(int j = 1; j <= N; j++) 
            {
                g2.fill(new Ellipse2D.Float(i - offset, j - offset, 2*radius, 2*radius));
            }
        }
    }
}